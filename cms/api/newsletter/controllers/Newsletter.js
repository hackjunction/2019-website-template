...
module.exports = {
    create: async (ctx, res) => {
        ...
        if (!strapi.config.currentEnvironment.sendgridApiKey) {
            return ctx.send({
                message: 'Sendgrid API key missing'
            });
        }
        if (!strapi.config.currentEnvironment.sendgridEmailList) {
            return ctx.send({
                message: 'No email list defined'
            });
        }
        ...
        return axios
            .post(
                ...
                ],
                {
                    ...
                    headers: {
                        Authorization:
                            'Bearer ' +
                            strapi.config.currentEnvironment.sendgridApiKey,
            ...
            .then(res => {
                if (
                    Array.isArray(res.data.persisted_recipients) &&
                    res.data.persisted_recipients.length > 0
                ) {
                    const recipientID = res.data.persisted_recipients[0];
                    const listID =
                        strapi.config.currentEnvironment.sendgridEmailList;
                    return axios.post(
            ...
            .catch(err => {
                console.log(
                    '/api/newsletters: SG ERR ADDING CONTACT',
                    err.response.data
                );
                ctx.send({
                    message: 'error'
